...
class ufront_web_UserAgent {
	...
	static function extractVersion($searchString, $s) {
		...
		return _hx_anonymous(array("version" => $tmp, "majorVersion" => $tmp1, "minorVersion" => Std::parseInt($re->matched(2))));
	}
	static function searchString($data, $s) {
		{
			...
			while($_g < $data->length) {
				...
				if(_hx_index_of($s, _hx_field($d, "subString"), null) >= 0) {
					...
					return _hx_anonymous(array("app" => _hx_field($d, "identity"), "versionString" => $tmp));
					unset($tmp);
				}
				unset($d);
			}
