<?php
    ...
    $at2x_filejoomla = pathinfo($source_file, PATHINFO_DIRNAME).'/'.pathinfo($source_file, PATHINFO_FILENAME).'2x.'.pathinfo($source_file, PATHINFO_EXTENSION);
    ...
    if (in_array($source_ext, array('png', 'gif', 'jpg', 'jpeg', 'bmp'))) {
        ...
        elseif ($cookie_value !== false && $cookie_value > 1) {
            ...
            elseif (file_exists($at2x_filejoomla)) {
                $source_file = $at2x_filejoomla;
                $status = 'retina image';
            }
        }

        // Check if we can shrink a larger version of the image
        if (!file_exists($source_file) && DOWNSIZE_NOT_FOUND && ($source_file !== $at2x_file) && ($source_file !== $at2x_filejoomla)) {
            // Check if retina image exists
            if (file_exists($at2x_file)) {
                $source_file = $at2x_file;
